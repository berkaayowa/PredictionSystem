		...
		if($object != null) {
			...
			if(!empty($object['controller']))
			    ...
			    if(!isset($object['prefix']['object']['home']))
			        throw new \Exception('Client controller for {'.$object['prefix']['name'].'} prefix is not set');

                $controller = ucfirst(strtolower($object['prefix']['object']['home']));

            }

            //Debug::PrintOut($object);
            ...
            throw new \Exception('System can process a null router object');
