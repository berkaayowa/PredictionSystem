...
class Form {
    ...
    private static function getInput($element) {
        switch($element['type']) {
            ...
            case 'hidden':
                ...
                break;
            case 'textarea':
                return '<textarea data-editor="true" rows="10" class="form-control {class}" {attributes}>{value}</textarea>';
                break;
            case 'number':
                return '<input type="number" class="form-control {class}" {attributes}>';
                break;
            case 'file':
    ...
    public static function radioButton($id, $value = null){
        $element ='
        <label class="radio-inline">
            <input type="radio" value="on" name="'.$id.'" id="'.$id.'" class="checkbox " '.($value == 1 ? 'checked' : '').'>'.Label::General('Yes').'
        </label>
        <label class="radio-inline">
            <input type="radio" value="off" name="'.$id.'" id="'.$id.'" class="checkbox " '.($value == 0 ? 'checked' : '').'>'.Label::General('No').'
        </label>
        ';
    ...
    public static function checkBox($id, $value = 0, $viewMode = false){


        return Html::select("", $id, [['x'=>1, 't'=>Label::General("Yes")], ['x'=>0, 't'=>Label::General("No")]],['value'=>'x', 'text'=>'t', 'selected'=>$value, $viewMode ? 'disabled' : '' => '']);
    }
