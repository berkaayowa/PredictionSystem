...
class Currency {
	...
	*/
    ...
    private function __construct($amount) {
        ...
        $this->amount = $amount;
        $this->base = @Auth::GetActiveUser(true)->currency->Code;
    ...
    public function updateCurrency($currency) {
        ...
        try {
            ...
            $currencyArray = [
                'symbol' => $currency,
                'rate' => $currency != $this->base && $resp != null && isset($resp->rates) ? $resp->rates->{$currency} : 1
            ];
    ...
    public static function getCurrentCurrency($code = 'ZAR') {

        if(!SessionHelper::exist('currency')){
            self::Init()->updateCurrency($code);
            return self::getCurrentCurrency($code);
        } else {
