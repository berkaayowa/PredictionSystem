...
class Html {
	...
	*/
    public static function select($label = "", $id,  $data = array(), $options) {
        $_label = !empty($label) ? self::label($label) : '';
        $i_select= "<select class='form-control' id='{$id}' name='{$id}' ";
        $i_option = '';
	...
	*/
    ...
    public static function action($link, $data = array()) {
        SessionHelper::add('request', $data);
